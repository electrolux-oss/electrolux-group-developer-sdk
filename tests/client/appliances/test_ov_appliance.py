...
from electrolux_group_developer_sdk.feature_constants import APPLIANCE_STATE, PROGRAM, TARGET_TEMPERATURE_C
...
def test_get_feature_state_string_options_string_cap(ov_appliance):
    assert len(ov_appliance.get_feature_state_string_options(APPLIANCE_STATE)) == 8


def test_get_feature_state_string_options_non_string_cap(ov_appliance):
    assert len(ov_appliance.get_feature_state_string_options(TARGET_TEMPERATURE_C)) == 0
