...
from ...appliance_config.dw_config import DwConfigManager, DwConfig, EXECUTE_COMMAND, EXECUTE_COMMAND_START, \
...
class DWAppliance(ApplianceData):
    """
    Extended appliance data class for DW appliances.

    Adds access to capabilities and state via DwConfig.
    """
    _config: DwConfig = PrivateAttr()

    def __init__(self, **data: Any):
        super().__init__(**data)
        capabilities = self.details.capabilities if self.details else {}
        self._config = DwConfigManager().get_config(self.appliance.applianceType, capabilities)
