...
from ...appliance_config.wd_config import WdConfig, WdConfigManager, EXECUTE_COMMAND, EXECUTE_COMMAND_START, \
...
class WDAppliance(ApplianceData):
    """
    Extended appliance data class for WD appliances.

    Adds access to capabilities and state via WdConfig.
    """
    _config: WdConfig = PrivateAttr()

    def __init__(self, **data: Any):
        super().__init__(**data)
        capabilities = self.details.capabilities if self.details else {}
        self._config = WdConfigManager().get_config(self.appliance.applianceType, capabilities)
