...
from ..constants import VALUES, WD, APPLIANCE_STATE_DELAYED_START
...
class WdConfig(ApplianceConfig):
    """Config for WD devices."""
...
class WdConfigManager:
    """Manager for WD device configuration by model."""

    def __init__(self) -> None:
        """Initialize with configuration data in WD_CONFIG."""
        self._config = WD_CONFIG

    def get_config(self, model_type: str, capabilities: dict[str, Any]) -> WdConfig:
        """Return the appliance_config for a given model type."""

        return WdConfig(mapping=self._config.get(model_type, {}), capabilities=capabilities)
