...
from ..constants import VALUES, DW, APPLIANCE_STATE_DELAYED_START
...
class DwConfig(ApplianceConfig):
    """Config for DW devices."""
...
class DwConfigManager:
    """Manager for DW device configuration by model."""

    def __init__(self) -> None:
        """Initialize with configuration data in DW_CONFIG."""
        self._config = DW_CONFIG

    def get_config(self, model_type: str, capabilities: dict[str, Any]) -> DwConfig:
        """Return the appliance_config for a given model type."""

        return DwConfig(mapping=self._config.get(model_type, {}), capabilities=capabilities)
